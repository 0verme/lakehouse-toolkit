...
class IncrementalPlannerTests(unittest.TestCase):
    ...
    def test_target_hint_selection_version_bump_rebuilds_v7_state(self):
        current = source("PROGRAM_DEMO_TARGET_HINT_SELECTION")
        previous = (
            replace(
                ProgramState.from_source(
                    current,
                ...
                pipeline_version="lineage-pipeline-v7-expanded-program-namespace-registry",
            ),
        )
